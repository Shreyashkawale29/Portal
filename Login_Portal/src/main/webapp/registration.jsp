...
<input type="hidden" id="status" value="<%= request.getAttribute("status")%>">
	...
	if(status == "InvalidName"){
		swal("Wrong","Please Enter Name","error")
	}
	
	if(status == "InvalidEmail"){
		swal("Wrong","Please Enter Email","error")
	}
	
	if(status == "InvalidPassword"){
		swal("Wrong","Please Enter Password","error")
	}
	
	if(status == "InvalidMobile"){
		swal("Wrong","Please Enter Mobile","error")
	}
	
	if(status == "InvalidMobileLength"){
		swal("Wrong","Mobile Number should be of 10 digits","error")
	}
	
	if(status == "InvalidConfirmPassword"){
		swal("Wrong","Password Not Matching","error")
	}
	</script>
